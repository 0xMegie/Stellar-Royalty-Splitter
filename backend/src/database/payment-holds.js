...
export function placeHold(transactionId, holdReason, holdUntil, placedBy) {
  ...
  const recordId = db.prepare("INSERT INTO hold_audit (transactionId, action, reason, performedBy, details) VALUES (?, 'placed', ?, ?, ?)").run(holdReason, placedBy, JSON.stringify({ holdUntil })).lastInsertRowid;
  countWrite();
...
export function releaseHold(transactionId, releasedBy, approvalNote) {
  const existing = db.prepare("SELECT * FROM transactions WHERE id = ? AND hold_status = 'active'").get(transactionId);
  if (!existing) return null;
  ...
  db.prepare("INSERT INTO hold_audit (transactionId, action, reason, performedBy, details) VALUES (?, 'released', ?, ?, ?)").run(transactionId, approvalNote ?? 'Released', releasedBy, JSON.stringify({ releasedAt: new Date().toISOString() }));
  countWrite();
...
export function approveHoldRelease(transactionId, approvedBy, approvalNote) {
  ...
  db.prepare("INSERT INTO hold_audit (transactionId, action, reason, performedBy, details) VALUES (?, 'approved', ?, ?, ?)").run(transactionId, 'Approved for release', approvedBy, JSON.stringify({ note: approvalNote }));
  countWrite();
...
export function getAllHeldTransactions(status = 'active') {
  return db.prepare(
    "SELECT * FROM transactions WHERE hold_status = ? ORDER BY hold_placed_at DESC"
  ).all(status);
}

export function getHoldAuditTrail(transactionId) {
  return db.prepare(
    "SELECT * FROM hold_audit WHERE transactionId = ? ORDER BY created_at ASC"
  ).all(transactionId);
}
